// ============================================================
// sensor.cpp — MPU6050 ISR-driven reading, calibration, I2C recovery
// ============================================================
#include "sensor.h"
#include <Arduino.h>
#include <Wire.h>
...
static void applyMountRotation(int16_t* ax, int16_t* ay, int16_t* az) {
    ...
    switch (s_calData.mount_mode) {
        case 0: break;  // Standard: no rotation
        case 1: *ax = by; *ay = -bx; *az = bz; break;  // 90° clockwise
        case 2: *ax = -bx; *ay = -by; *az = bz; break;  // 180°
        case 3: *ax = -by; *ay = bx; *az = bz; break;  // 270°
    }
}
...
bool initMPU6050() {
    ...
    Serial.printf("[SENSOR] %s found (WHOAMI=0x%02X)\n",
                  (whoami == 0x68 ? "MPU6050" : "MPU6500"), whoami);
    ...
    writeMPURegister(MPU_REG_INT_ENABLE, 0x01);
    // INT pin config
    writeMPURegister(0x37, 0x00);
...
bool readSensorBurst(SensorSample* sample) {
    ...
    if (s_calData.temp_coeff != 0) {
        ...
        float deltaT = tempC - 25.0f;
        int16_t correction = (int16_t)(deltaT * (s_calData.temp_coeff / 1000.0f));
        cal_gx += correction;
        cal_gy += (int16_t)(correction * 9 / 10);
        cal_gz += (int16_t)(correction * 8 / 10);
    }
    ...
    s_lastAccel[0] = sample->accel_x; s_lastAccel[1] = sample->accel_y; s_lastAccel[2] = sample->accel_z;
    s_lastGyro[0] = sample->gyro_x;   s_lastGyro[1] = sample->gyro_y;   s_lastGyro[2] = sample->gyro_z;
...
void getLastAccelGyro(int16_t* accel, int16_t* gyro) {
    if (accel) { accel[0] = s_lastAccel[0]; accel[1] = s_lastAccel[1]; accel[2] = s_lastAccel[2]; }
    if (gyro)  { gyro[0] = s_lastGyro[0];  gyro[1] = s_lastGyro[1];  gyro[2] = s_lastGyro[2]; }
}
