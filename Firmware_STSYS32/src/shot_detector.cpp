// ============================================================
// shot_detector.cpp — Shot Detection State Machine (Firmware-side)
// Ports the IDLE→ARMING→ARMED→POST_GATHER→COOLDOWN logic from Python/Flutter
// ============================================================
...
// Local config copy (updated from ISR-safe config)
static DetectorConfig s_localCfg;

// ================= PIEZO FILTERING =================
// IIR low-pass filter: filtered = filtered * 0.9 + raw * 0.1
static float s_piezoFilter = 0.0f;

// Previous accel for jerk computation
...
void initShotDetector() {
    ...
    memcpy(&s_localCfg, &g_detectCfg, sizeof(DetectorConfig));
}

void updateShotDetectorConfig(const DetectorConfig* cfg) {
    if (cfg == NULL) return;
    memcpy(&s_localCfg, cfg, sizeof(DetectorConfig));
}
...
// ================= COMPUTE JERK =================
static int16_t computeJerkMag(int16_t ax, int16_t ay, int16_t az) {
    int16_t jerkX = abs(ax - s_prevAccelX);
    int16_t jerkY = abs(ay - s_prevAccelY);
    int16_t jerkZ = abs(az - s_prevAccelZ);
    return jerkX + jerkY + jerkZ;
}

// ================= PROCESS SAMPLE =================
// Returns true if a shot was detected, fills eventOut
bool processSample(const SensorSample* sample, ShotEvent* eventOut,
    ...
    // Compute Jerk
    int16_t jerkMag = computeJerkMag(sample->accel_x, sample->accel_y, sample->accel_z);
    ...
    uint16_t effectivePiezoThreshold = s_localCfg.piezo_threshold;
    if (s_localCfg.adaptive_threshold_enabled && g_detectState.adaptive_threshold > 0) {
        effectivePiezoThreshold = g_detectState.adaptive_threshold;
    }

    // Detection: Both thresholds must pass
    bool piezoPass = (filteredPiezo >= effectivePiezoThreshold);
    ...
    if (piezoPass && accelPass) {
        ...
        // Peak accel (magnitude comparison)
        int32_t curAccelMagSq = (int32_t)sample->accel_x * sample->accel_x +
                                 (int32_t)sample->accel_y * sample->accel_y +
                                 (int32_t)sample->accel_z * sample->accel_z;
        int32_t maxAccelMagSq = (int32_t)g_detectState.maxAccelX * g_detectState.maxAccelX +
                                 (int32_t)g_detectState.maxAccelY * g_detectState.maxAccelY +
                                 (int32_t)g_detectState.maxAccelZ * g_detectState.maxAccelZ;
        if (curAccelMagSq > maxAccelMagSq) {
            g_detectState.maxAccelX = sample->accel_x;
            g_detectState.maxAccelY = sample->accel_y;
            g_detectState.maxAccelZ = sample->accel_z;
        }

        // Peak gyro (magnitude comparison)
        int32_t curGyroMagSq = (int32_t)sample->gyro_x * sample->gyro_x +
                                (int32_t)sample->gyro_y * sample->gyro_y +
                                (int32_t)sample->gyro_z * sample->gyro_z;
        int32_t maxGyroMagSq = (int32_t)g_detectState.maxGyroX * g_detectState.maxGyroX +
                                (int32_t)g_detectState.maxGyroY * g_detectState.maxGyroY +
                                (int32_t)g_detectState.maxGyroZ * g_detectState.maxGyroZ;
        if (curGyroMagSq > maxGyroMagSq) {
            g_detectState.maxGyroX = sample->gyro_x;
            g_detectState.maxGyroY = sample->gyro_y;
            g_detectState.maxGyroZ = sample->gyro_z;
        }
    ...
    if (g_detectState.inWindow) {
        ...
        // Close window after 5ms or max samples
        if (windowDuration >= 5000 || g_detectState.windowCount >= DETECTION_WINDOW * 2) {
            if (g_detectState.windowCount >= 2) {
                ...
                if (sample->timestamp_us - g_detectState.lastShotTime_us >= debounceUs) {
                    ...
                    // Update adaptive threshold ring buffer
                    ...
                    // Compute running mean + stddev for adaptive threshold
