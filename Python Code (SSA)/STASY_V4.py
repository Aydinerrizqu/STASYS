...
DEFAULT_PIEZO_MIN = 400.0    # TUNED: more reliable for dry fire
PIEZO_MAX_LIMIT = 2500.0      # TUNED: allow stronger dry fire
...
def parse_binary_packet(ser):
    try:
        ...
        if ser.in_waiting >= PACKET_SIZE:
            ...
            # Packet sekarang: ax,ay,az,gx,gy,gz,piezo,battery,state_flag
                    ...
                    while time.time() - st_resp < 2.0:
                        if ser.in_waiting:
                            ...
                            if resp and resp != "READY":
                                ...
                                if hmac.compare_digest(resp.lower(), exp.lower()):
                                    return True
                        time.sleep(0.01)
            time.sleep(0.05)
    except Exception as e:
...
class ShotDetector:
    """
    TRUE MANTIS-X BEHAVIOR:
    =======================
    
    ANCHOR SYSTEM:
      - Saat ARMED: anchor di-UPDATE TERUS (rolling average, smoothing)
      - Saat trigger: anchor FREEZE
      - Setelah shot selesai: anchor RESET ke curr (siap shot baru)
      
    TRACE INTEGRATION:
      - curr_x / curr_y = posisi absolut (terus integrate)
      - Setiap kali re-arm: trace di-RECENTER (curr - anchor -> 0)
      
    DRIFT HANDLING:
      - Firmware: drift correction STOP saat ARMED/ARMING/POST_GATHER
      - Python: anchor update MENGKOMPENSASI drift kecil
      
    DISPLAY:
      - Live trace = curr - anchor
      - Center (0,0) = anchor = point of aim
      - Saat ARMED dan diam: cursor stabil di (0,0) ± tremor
    """
    
    def __init__(self):
        self.accel_thresh = DEFAULT_ACCEL_THRESH
        self.piezo_thresh = DEFAULT_PIEZO_MIN
        # trigger_mode: 0 = Piezo (dry fire), 1 = Jerk (live fire / piezo backup)
        # FALLBACK: Kalau piezo tidak jalan, ganti ke 1
        self.trigger_mode = 0  # Change to 1 if piezo broken
        ...
        self.anchor_x = 0.0
        self.anchor_y = 0.0
        self.anchor_frozen = False  # Flag: anchor di-freeze saat trigger
        
        # Anchor smoothing (rolling average saat ARMED)
        self.anchor_alpha = 0.02  # Smoothing factor
    ...
    def calibrate(self, samples):
        """Reset trace dan anchor ke origin."""
        if len(samples) < 10:
            return False
    ...
    def process(self, packet):
        # Packet: ax,ay,az,gx,gy,gz,piezo,bat,state_flag
        raw_ax, raw_ay, raw_az, raw_gx, raw_gy, raw_gz, piezo, bat = packet
        
        # Gyro sudah bersih dari firmware
        ...
        if self.state == "COOLDOWN":
            ...
            if self.state_timer <= 0:
                # Shot selesai: RESET anchor ke posisi sekarang
                # Siap untuk shot baru dengan baseline baru
                self.anchor_x = self.curr_x
                self.anchor_y = self.curr_y
                self.anchor_frozen = False
                self.state = "IDLE"
            ...
            if rot_mag > STABILITY_GYRO_LIMIT:
                ...
                # Selama ARMING: smooth anchor ke curr (averaging)
                self.anchor_x += self.anchor_alpha * (self.curr_x - self.anchor_x)
                self.anchor_y += self.anchor_alpha * (self.curr_y - self.anchor_y)
                
                if self.state_timer >= STABILITY_WINDOW_MS:
                    self.state = "ARMED"

        elif self.state == "ARMED":
            # CRITICAL: CONTINUOUS ANCHOR UPDATE saat ARMED
            # Ini yang bikin trace tidak drift saat diam
            if not self.anchor_frozen:
                self.anchor_x += self.anchor_alpha * (self.curr_x - self.anchor_x)
                self.anchor_y += self.anchor_alpha * (self.curr_y - self.anchor_y)

            # DEBUG: Uncomment untuk lihat sensor values saat ARMED
            print(f"[ARMED] Piezo:{piezo:4d} Rot:{rot_mag:5.2f} Jerk:{jerk_mag:5.1f} Thresh:{int(self.piezo_thresh)}")

            # Trigger detection
            triggered = False
            if self.trigger_mode == 1:
                # Live Fire / FALLBACK: juga bisa backup kalau piezo broken
                if jerk_mag > (self.accel_thresh * 1.5):
                    triggered = True
            else:
                # Dry Fire detection
                if piezo >= self.piezo_thresh and piezo <= PIEZO_MAX_LIMIT:
                    if rot_mag < 10.0:  # TUNED: allow more movement during trigger
                        triggered = True

            if triggered:
                # FREEZE ANCHOR saat trigger
                self.anchor_frozen = True
                self.last_trigger_piezo = piezo
                self.state = "POST_GATHER"
                self.gather_counter = RECOIL_DURATION_IDX

            if rot_mag > (STABILITY_GYRO_LIMIT * 3.0):
                self.state = "IDLE"
    ...
    def analyze_shot(self):
        """Semua segment relative ke FROZEN ANCHOR."""
        ...
        # Normalize ke FROZEN ANCHOR
        def get_norm_segment(start, end):
            seg_x = [x - self.anchor_x for x in full_x[start:end]]
            seg_y = [y - self.anchor_y for y in full_y[start:end]]
            return seg_x, seg_y
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # RIGHT PANEL
        right_panel = QVBoxLayout(); right_panel.addWidget(QLabel("Session History"))
        ...
        self.btn_reset = QPushButton("New Session"); self.btn_reset.clicked.connect(self.reset_session)
    ...
    def reset_session(self):
        ...
        self.lbl_big_score.setText("--"); self.curve_hold.clear(); self.curve_press.clear(); self.curve_recoil.clear(); self.hit_marker.clear()
    ...
    def update_loop(self):
        ...
        while (hasattr(self.ser, 'update_sim') or self.ser.in_waiting >= PACKET_SIZE) and count < 10:
            ...
            if recent_x:
                # Anchor-based display (Mantis-X)
                ax = self.detector.anchor_x
                ay = self.detector.anchor_y

                if self.detector.is_calibrated:
                    short_x = recent_x[-50:]; short_y = recent_y[-50:]
                    disp_x = [x - ax for x in short_x]
                    disp_y = [y - ay for y in short_y]
                    self.live_curve.setData(disp_x, disp_y)
                    # Cursor = posisi sekarang relative ke anchor
                    self.cursor.setData([recent_x[-1] - ax], [recent_y[-1] - ay])

                if self.live_window and self.live_window.isVisible():
                    long_x = recent_x[-100:]; long_y = recent_y[-100:]
                    mon_x = [x - ax for x in long_x]
                    mon_y = [y - ay for y in long_y]
